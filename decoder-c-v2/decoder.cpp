    ...
    for (uint y = image->height - 1; y < image->height; --y) {
        ...
        for (uint x = 0; x < image->width; ++x) {
            ...
            *bufferPos++ = image->blocks[blockIndex].r[pixelIndex];
            if(x == 22 && y == 22){
                std::cout<<"BGR values are"<< std::endl;
                std::cout << image->blocks[blockIndex].b[pixelIndex] << " ";
                std::cout << image->blocks[blockIndex].g[pixelIndex]<< " ";
                std::cout << image->blocks[blockIndex].r[pixelIndex]<< " ";
            }
        }
